 ...
 * May throw [IndexOutOfBoundsException] if index is negative or greater than buffer size.
 */
inline operator fun Buffer.get(index: Long): Byte = loadByteAt(index)

/**
 * Index write operator to write [value] at the specified [index].
 * May throw [IndexOutOfBoundsException] if index is negative or greater than buffer size.
 */
inline operator fun Buffer.set(index: Long, value: Byte): Unit = storeByteAt(index, value)

/**
 * Index write operator to write [value] at the specified [index].
 * May throw [IndexOutOfBoundsException] if index is negative or greater than buffer size.
 */
inline operator fun Buffer.set(index: Int, value: Byte): Unit = storeByteAt(index, value)
 ...
 * Copying bytes from a buffer to itself is allowed.
 */
fun Buffer.copyTo(destination: Buffer, offset: Long, length: Long, destinationOffset: Long): Unit = copyTo(
    destination,
 ...
 */
fun Buffer.toByteArray(offset: Int, length: Int): ByteArray {
